	...
	uuidList.forEach((uuid, key) => {
      ...
      let buttonContext = window["iamportButtonContext_" + uuid];
		...
		$(uuidToStringForPay).click(() => {
			...
			const parsedAmount = parseFloat( raw_amount.replace(/\(.*?\)/g, '').replace(/[^0-9\.]/g, '') ); //() 안에 들어있는 라벨 먼저 제거하고 숫자만 골라냄( 라벨 내에 숫자가 있을 수도 있으니까 )
			if (isNaN(parsedAmount)) {
				inputValues[ amountField.getName() ] = -1; //에러처리를 위해 마이너스 금액을 전달
			} else {
				inputValues[ amountField.getName() ] = parsedAmount;
			}
...
function iamportAjaxCall($, iamportBox, fileFields, inputValues, buttonContext, extraFields) {
	...
	const order_amount = inputValues.order_amount;
