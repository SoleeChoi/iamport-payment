...
if ( isset($_POST['action']) && $_POST['action'] === "migrate_to_iamport_block") {  
  ...
  ?>
    <script type="text/javascript">
      window.location.href = "<?=admin_url('edit.php?post_type=iamport_payment')?>";
    </script>
  <?php
}

if ( !class_exists('IamportPaymentShortcode') ) {
	require_once(dirname(__FILE__).'/iamport.php');
	require_once(dirname(__FILE__).'/iamport-order.php');
	require_once(dirname(__FILE__).'/iamport-payment-button.php');
    ...
    private $callback;
      ...
      add_submenu_page(
		...
		public function init() {
			...
			wp_register_style('iamport-payment-shortcode-css', plugins_url('../assets/css/iamport-payment-shortcode.css', __FILE__), array(), "20180730");
			wp_enqueue_style('iamport-payment-shortcode-css');

			wp_register_script('iamport-bundle-main-js', plugins_url('../dist/bundle-main.js', __FILE__), array('jquery'), '20190105');
			wp_enqueue_script('iamport-bundle-main-js');
		}
		...
		public function ajax_get_order_uid() {
			...
			if ( preg_match('/[0-9]+m/', $iamportSetting['card_max_quota']) ) {
				$card_max_quota = range(1, intval($iamportSetting['card_max_quota']));
			}

			$response = array('order_id'=>$order_id, 'order_uid'=>$order_uid, 'thankyou_url'=>$thankyou_url, 'vbank_due'=>$vbank_due);
			if ($card_max_quota) {
				$response['display'] = array('card_quota' => $card_max_quota);
			}
				...
				case 'attached_files': {
					...
					if ( empty($attachedFiles) ) echo '-';
						...
						foreach ( $attachedFiles as $file => $fileData ) {
							...
							$files[] = sprintf('<a style="text-decoration: underline;" href="%s" download="%s" onmouseover="hoverOnFile(event)" onmouseleave="leaveOnFile(event)"><img src="%s"/></a><div class="file-info"><p>%s</p><p class="file-name-size">%s/%s</p></div>',
								$fileData['location'],
								$normalizedFileName,
